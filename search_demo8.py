...
class BertAISearchModel(nn.Module):
    ...
    def forward(self, ids, mask):
        ...
        one_output = self.dense(pooled_output)
...
def analysis(out, label, name):
    ...
    print(np.mean(list0))
    print(np.mean(list1))
    list0Index = [i for i in range(len(list0))]
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for data2 in tqdm(val_loader):
            ...
            # print(out_matrix2)
            # print(label_matrix2)
            loss2, current_num2 = lossFunction(out_matrix2, label_matrix2, 0, SAMPLE_NUM, valid_log_name)
            # print(loss2)
            # print(current_num2)
            # 计算准确率
