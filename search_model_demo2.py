...
def get_train_valid_data(query, document, t_size, v_size):
    """
    获取训练集，验证集
    :param query:
    :param document:
...
def get_test_data(query, document, num):
    return query[:num], document[:num], np.eye(num)
...
#test_dataset = DataToDataset(test_query, test_document, test_label, tokenizer, max_length)
...
for epoch in range(epochs):
    ...
    log_str = str(epoch) + "   mean_loss  " + str(mean_loss) + "  accuracy  " + str(accuracy)
    write_txt_result("log1.txt", log_str)
    ...
    with torch.no_grad():
        ...
        log_str2 = str(epoch) + "   mean_loss  " + str(mean_loss2) + "  accuracy  " + str(accuracy2)
        write_txt_result("log1.txt", log_str2)
