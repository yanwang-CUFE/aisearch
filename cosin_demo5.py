...
from utils import json_to_dict, write_txt_result, get_data2, drawScore, drawSocre2, drawScore3, get_data3, get_data4
...
all_valid_query, all_valid_document, all_valid_label = get_sample_data(all_valid_query, all_valid_document, 233, VALID_SIZE)
...
dict_to_json(valid_data_dict, 'valid41.json')
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for data2 in tqdm(val_loader):
            ...
            #
            # print(labels_list2)
            out_matrix2 = torch.zeros([2, 233, 312, Valid_BATCH_SIZE])

            label_matrix2 = torch.zeros([233, 233, Valid_BATCH_SIZE])
            # 优化器置零
            # optimizer.zero_grad()
            ...
            # print(out_matrix2)
            # print(label_matrix2)
            loss2, current_num2 = lossFunction2(out_matrix2, labels_list2, 0, 233, valid_log_name)
            # print(loss2)
            # print(current_num2)
            # 计算准确率
    ...
    # ("准确率")
    accuracy2 = correct_predictions2.double() / (VALID_SIZE * 233)
    # ("平均损失")
