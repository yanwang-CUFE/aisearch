...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for data2 in tqdm(val_loader):
            ...
            #
            # print(labels_list2)
            out_matrix2 = torch.zeros([2, 5, 312, Valid_BATCH_SIZE])

            label_matrix2 = torch.zeros([5, 5, Valid_BATCH_SIZE])
            # 优化器置零
            # optimizer.zero_grad()
            ...
            # print(out_matrix2)
            # print(label_matrix2)
            loss2, current_num2 = lossFunction2(out_matrix2, labels_list2, 0, 5, valid_log_name)
            # print(loss2)
            # print(current_num2)
            # 计算准确率
    ...
    # ("准确率")
    accuracy2 = correct_predictions2.double() / (VALID_SIZE * 5)
    # ("平均损失")
