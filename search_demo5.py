...
for epoch in range(epochs):
    ...
    for data in tqdm(train_loader):
        ...
        print(labels_list)
        print("tokenizer----------")
        for ind, l in enumerate(input_ids_list):
            print("--")
            print(ind)

            for ind2, l2 in enumerate(l):
                print(ind2)
                print(tokenizer.decode(l2))

            print("--end")

        out_list = []

        out_matrix = torch.zeros([len(input_ids_list), Train_BATCH_SIZE])

        label_matrix = torch.zeros([len(input_ids_list), Train_BATCH_SIZE])
        # 优化器置零
        optimizer.zero_grad()
        for index, i in enumerate(input_ids_list):
            print("in for each in list")
            # print(index)
            ...
            print(input_ids_list[index])
            print(attention_mask_list[index])
        ...
        lossFunc = PairwiseMaxMarginHingeLossFunc(Train_BATCH_SIZE).to(device)
    ...
    if (epoch % 50 == 1):
        ...
        write_txt_result("log5.txt", log_str)
        ...
        #
        # with torch.no_grad():
        #     valid_txt = "./train5/valid" + str(epoch) + ".txt"
        ...
        #     write_txt_result("log5.txt", log_str2)
