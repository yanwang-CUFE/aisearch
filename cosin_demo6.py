...
from utils import json_to_dict, write_txt_result, get_data2, drawScore, drawSocre2, drawScore3, get_data3, get_data4, get_data5
...
file_name = "indexXML_1106_nodoc2.json"

all_query, all_document = get_data5(file_name)
all_train_query, all_train_document, all_valid_query, all_valid_document = get_all_train_valid_data(all_query, all_document, 4261, 1064)
...
all_valid_query, all_valid_document, all_valid_label = get_sample_data(all_valid_query, all_valid_document, 1000, VALID_SIZE)
...
dict_to_json(valid_data_dict, 'valid44.json')
...
for epoch in range(epochs):
    ...
    for data in tqdm(train_loader):
        ...
        # print(out_matrix)
        # print(label_matrix)
        loss, current_num = lossFunction(out_matrix, labels_list, 0, SAMPLE_NUM, train_log_name)
        # print(loss)
        # print(current_num)
        # 计算准确率
        losses.append(loss)
    ...
    with torch.no_grad():
        for data2 in tqdm(val_loader):
            ...
            #
            # print(labels_list2)
            out_matrix2 = torch.zeros([2, 1000, 312, Valid_BATCH_SIZE])

            label_matrix2 = torch.zeros([1000, 1000, Valid_BATCH_SIZE])
            # 优化器置零
            # optimizer.zero_grad()
            ...
            # print(out_matrix2)
            # print(label_matrix2)
            loss2, current_num2 = lossFunction2(out_matrix2, labels_list2, 0, 1000, valid_log_name)
            # print(loss2)
            # print(current_num2)
            # 计算准确率
    ...
    # ("准确率")
    accuracy2 = correct_predictions2.double() / (VALID_SIZE * 1000)
    # ("平均损失")
    ...
    if accuracy2 > max_eval_rate:
        ...
        torch.save(model_to_save.state_dict(), "./model_save2/model5.bin")
        torch.save(mymodel, "./model_save2/mymodel5.pt")
        print("保存成功")
